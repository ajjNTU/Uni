    ...
    while i < len(values)-1:
        ...
        while j < len(values):
            ...
            if values[i] == values[j]:
                #print("Removing: ", values[j], "because matches", values[i])
                values.remove(values[i])
                continue
            else:
                #print("OK: ", values[i], "and", values[j], "don't match")
                j += 1
        i += 1
...
removeDuplicates( ["oranges", "oranges", "apple", "bananas", "organges", "apple", "apple", "apple", "bananas","bananas","bananas", "organges"] )
